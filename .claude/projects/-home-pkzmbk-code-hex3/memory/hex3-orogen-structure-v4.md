...
**MERGED TO MAIN 2026-06-21** (commit 731abb2): emergent is now the DEFAULT (flipped FINE_EMERGENT_LAMBDA=0.5, FINE_EMERGENT_STRUCTURED=1, EROSION_N=2, EROSION_HILLSLOPE_CRITICAL_SLOPE=200, EROSION_STEPS=60→200, channel_support=4; painted P1 knobs zeroed → A/B baseline via knobs). PERF: default gen ~3× slower (n=2 Newton + 200 steps) — `steps` is the quality↔speed dial. Cache FINE_BASE_CACHE_VERSION 9. The whole line (sweep harness, hillshade fix, P1, n>1, O0) landed in this merge. Per [[hex3-sweep-image-reading]], terrain aesthetics are the USER's call (numbers necessary-not-sufficient); judged via Windows hillshade sweeps ([[hex3-sweep-harness]]).

REMAINING/FUTURE: (1) coarse-asymmetry single-owner redesign (C/A hybrid) for atmosphere rain-shadow consistency — deferred; (2) wide-mountain residual spikiness (structural, not an erosion knob); (3) the EROSION_STEPS=200 perf cost may warrant revisiting.

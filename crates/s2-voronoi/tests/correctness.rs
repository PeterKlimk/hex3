...
fn test_cell_areas_reasonable() {
    ...
    for cell in output.diagram.iter_cells() {
        if cell.len() >= 3 {
            ...
            assert!(cell.len() <= 20, "cell has too many vertices: {}", cell.len());
        }
    }
...
fn test_reproducibility() {
    ...
    assert_eq!(output1.diagram.num_vertices(), output2.diagram.num_vertices());
...
fn test_knn_matches_qhull_structure() {
    ...
    let points: Vec<UnitVec3> = fibonacci_sphere_points(100, 0.05, 44444);

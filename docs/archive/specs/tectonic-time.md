...
**T4e continuous boundary work implemented; full Eulerian transport rejected.** Two
literal fractional-material prototypes were evaluated and removed. Single-owner
finite-volume transport mixed all fourteen motion domains into one and made 65% of land
mountainous. Persistent per-plate Eulerian layers avoided that ownership collapse but
diffusively smeared inherited roots and magma around the sphere over 100 Myr, with a
20–60 second 8k–16k solve cost. Neither lies on the intended authenticity/compute
frontier.

The retained hybrid uses the topology-aware conservative pullback only to select visible
surface ownership. Physical creation, consumption, underthrust, and retained magma are
integrated independently from face-swept area. Remap losers enter conservative material
deposit ledgers and cannot trigger deformation. Undeformed base columns are intensive on
their surviving same-plate support; explicit buried-sheet and batholith reservoirs remain
extensive and spatially bounded. A 0.45-cell CFL cap replaces cell-crossing event timing,
and a unit invariant requires nonzero work below one-cell displacement and linearity
under time subdivision. Canonically sorted simultaneous merges restore bit determinism.

This fixes the binary-event dead zone and local remap stacking, but does not promote the
model. Seed 12345 at 4k/8k/16k gives peaks 12.55/9.23/10.64 km and land
38.3/30.0/29.7%, failing both standing resolution gates. At canonical 8k all ten seed
peaks pass (median 8.75 km), while two seeds still turn 41.4% and 49.8% of land into
mountains. Halving duration sharply reduces both plateaus. The configured 25 Myr motion
coherence has no effect because this forward solver still consumes constant initial
Euler poles rather than history snapshots.

**T4f is therefore motion-history integration, not response tuning.** Each lifecycle
interval must use the contemporaneous Euler poles for pullback, face work, and collision
coupling. Merge-induced poles remain solver state and must compose deterministically with
scheduled reorganizations. The next A/B must show that finite coherence changes path and
load duration, closes all ledgers, preserves rigid-motion identity, and improves the two
plateau outliers without a height or area cap.

...
| Scalar tectonic final-height ownership | **Replace; regional-support mechanism slice provisionally positive** | Keep the legacy output as the usable product baseline and control | Static nearest-front support removes the false comb but leaves a broad mound. RDS0b's exact-overlap, time-indexed pair creates real linked lobes/lows/ends and changes support drainage, but also possible perforated/bead grammar and a surviving roof. Human review plus one source→highland→drainage relationship readout decides whether it earns broader confirmation; H/C/G/I, inheritance expansion and Legacy tuning do not advance. |

...
| Scalar tectonic final-height ownership | **Replace; RDS0 moving-support ownership rejected; channel/hillslope dual selected as bounded upper bound** | Keep the legacy output as the usable product baseline and control; retain RDS0 source and conservative overlap infrastructure | Static nearest-front support leaves a broad mound. RDS0b creates visible lobes/lows/ends, but the fixed relationship readout finds 100 support depressions versus 3 in control, 86 cutting the >1.5 km highland, no represented major-river cells in highland components, and one merged 8.76 million km² highland. Moving positive support is not sufficient terrain ownership. The next slice separates drainage scaffold, sparse channels and non-channel surface; H/C/G/I, inheritance expansion and Legacy tuning do not advance. |

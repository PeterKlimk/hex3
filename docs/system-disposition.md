...
| Scalar tectonic final-height ownership | **Replace; regional-support mechanism slice selected** | Keep the legacy output as the usable product baseline and control | The ordinary-world corpus, finite-age coupling and corrected-flux A/B establish both product harm and a responsive downstream path. Static nearest-front support removes the false comb but leaves a broad mound. Test one conservative time-indexed regional deformation counterfactual before choosing a history generator; H/C/G/I, inheritance expansion and Legacy tuning do not advance. |

...
| Consequential/human geography | **Selected bounded expansion** | Traversability, access, named relative opportunities, routes and aggregate sites make retained geography consequential and produce board-like stories | The [on-demand V0 causal slice](consequential-geography.md) now has provisional sites and accepted terrain-sensitive route corridors at low cost. Test one honest route-local explanation before agents, economies or culture; stop rather than invent labels if current evidence cannot support it. |

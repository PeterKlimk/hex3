...
| B0 | A channel/hillslope dual can turn retained deformation opportunity into hydrologically compatible regional relief without a long whole-surface simulation | Same fixed RDS0 world/source and exact transfer; all-cell drainage scaffold distinct from sparse channels; descending long profiles; smooth non-channel reconstruction; one product reroute/reconciliation and relationship packet | **Selected, not implemented.** Explicit authentic-hack upper bound after RDS0 rejection. Must not reuse graph-depth G, inject moving scalar opportunity as height, redistribute uplift with A4, author an independent ridge graph or force every basin to an exterior outlet |

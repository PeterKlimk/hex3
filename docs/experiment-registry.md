...
It summarizes code and recorded evidence through 2026-07-23; it is not a substitute
...
| FHC4 | Does reduced coevolution add useful range/drainage organization beyond the same compact finite-parent forcing? | One H and one C run at 4 km; identical F field and static inputs; common S0/D0/O0a evidence and sharp physical view; no sweep or promotion packet | Completed negative discriminator: C adds real denudation and longer portal trunks, but both surfaces remain smooth elongated massifs and C does not earn `746.10 s` versus H's `336.43 s`. Whole-domain critical-point counts are not massif-local evidence. H's repeated target restoration makes this a mechanism test, not a fair owner tournament. Mountain branch parked by the cross-system disposition |
| FA-S1 | Finite-age uplift on exact frozen present-front support lets drainage and hillslopes express unequal belt maturity | Full Legacy convergent demotion; one exact nearest-front owner; retained episode age and local positive convergence; existing Stage-4 response | Implemented and research-selectable at `6010d3c`; three-world physical/visual and source-to-response gates pass, but within-range comb cadence is inherited from edge-local source rates; not promoted |
| FA-CF1 | Signed aggregation before positive classification removes false Voronoi-edge rectification grammar without changing topology or downstream response | Fixed one-collision-width conservative finite-volume operator; raw Slice A control; same globally normalized Legacy-derived terrain budget | Implemented at `3056069`; signed source ledger closes near roundoff and the comb is removed. Human review finds the coupled result over-smooth and mound-like. Retain as necessary source operation, reject static nearest-front support as sufficient terrain architecture |
| RDS0 | Finite growth, overlap/linkage, retirement and cross-front movement are the missing intermediate consequences between corrected work and erosion | RDS0a source plus RDS0b: seed `8675309`, episode `9`, 100k coarse source, ordinary ~250k process mesh, one exact-overlap map, identical continuous four-block Legacy budget, matched terrain and relationship overlays | Source and transfer mechanisms retained, terrain composition rejected. Exact overlap closes 2,877 donors through 83,559 pairs and injected budgets match, but the relationship readout finds 100 support depressions versus 3 in control, 86 cutting >1.5 km highland, several 3–6.5 km deep, one merged 8.76 million km² highland, and no represented major-river cell in any highland component. Visible lobes/lows are insufficient; no promotion, generated prior, other-world confirmation or tuning |
| B0 | A channel/hillslope dual can turn retained deformation opportunity into hydrologically compatible regional relief without a long whole-surface simulation | Same fixed RDS0 world/source and exact transfer; all-cell product receiver scaffold; 7,938 sparse high-order/fabric-conditioned channel cells; `S*=(u/A^m)^(1/n)` profiles integrated from real base levels; finite-volume steady hillslope reconstruction; one solved amplitude under Legacy-derived solid and 4.00 km relief caps; final product hydrology and matched relationship packet | **Implemented and rejected as sufficient ownership.** All promoted edges are monotonic and the RDS0 depression grammar nearly disappears, but final hydrology retains only 41.59% of support receivers and 58.01% on promoted channels, 91,151 cells reroute, only 2.60% of the positive relief budget remains while 7.08 million km³ is removed, no highland component contains a represented major-river cell, and matched views are mostly flat with isolated narrow remnants. Retain operator/diagnostic seams; do not tune or promote the composition |
...
manufactured I do not advance. The
[cross-system disposition](system-disposition.md) parks this branch; when
mountain work resumes, the next principled seam is causal forcing from real

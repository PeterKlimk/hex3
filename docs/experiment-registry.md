...
| RDS0 | Finite growth, overlap/linkage, retirement and cross-front movement are the missing intermediate consequences between corrected work and erosion | RDS0a: one seed `8675309`, episode `9`, 100k authoritative coarse source raster, identical corrected parent work and age envelope; conditional next gate: conservative transfer to the ordinary process mesh and one fixed Legacy-budget terrain pair | RDS0a implemented source-only: selected 342-edge collision parent, four equal frames, zero omissions, program/mesh closure at `1.4e-9 km²/Myr` or better, explicit 50/50 sides and additive overlap. Narrow representation pass only; no terrain, generated-history prior, broad campaign or promotion yet |

...
| `hydrology.r1_affine_centroid_gradient_error.v1` | evaluated prerequisite | all-cell maximum relative vector error and sine-angle error of the frozen polygon-centroid least-squares gradient against analytic affine A | ratios; every retained cell, maximum per registration | 11/12 registrations meet `1e-10`; one 2 km case reaches `1.102e-10` relative and `1.031e-10` sine, so its X1 trace is not judged |
| `hydrology.r1_continuous_crossing_geometry.v1` | evaluated manufactured-path gate | semantic-portal termination, maximum analytic cross-track, arclength error and backtracking of actual polygon-segment intersections from a prescribed physical head | km/ratio plus typed termination; one X0 or judged X1 path | X0 passes 12/12 and X1 passes 11/11 judged; does not establish a product thalweg, flux field or the censored X1 case |
| `hydrology.r1_internal_score_equivalence.v1` | evaluated causal prerequisite | exact eligibility and best-internal-face conflicts between affine generator two-point scores and reconstructed-vector connector scores, with normalized margins and score error | conflict counts plus ratios; all directed internal faces and per-cell winners | zero eligibility and winner conflicts in 12/12; excludes portal scoring and cannot override the failed reconstruction prerequisite |
| `hydrology.r1_affine_registered_row_defect.v1` | evaluated numerical diagnostic | difference between a registered polygon-mean neighbor row `(z_j-z_i)/h` and direct analytic affine centroid row `g* dot r_ij`, with grade and row-length normalizations | grade plus dimensionless ratios; all directed internal rows | maximum absolute `1.139e-12`, grade-normalized `1.139e-10`; combines polygon moments, centroid arithmetic and elevation subtraction rather than identifying one subcause |
| `hydrology.r1_affine_stable_solve_effect.v1` | evaluated causal diagnostic | maximum gradient difference between fixed-order normal equations and streaming Givens QR for the same registered or oracle rows, normalized by analytic grade | ratio; all retained cells per registration | registered maximum `1.044e-15`; QR does not change the 11/12 gate, while both direct-row oracle solves pass 12/12 |

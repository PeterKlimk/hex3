...
| `hydrology.seeded_face_path_cross_track_max_km.v1` | provisional R1a gate | maximum absolute analytic cross-track coordinate over prescribed head plus selected shared-face midpoints and portal-face midpoint | km; path vertices | resolved-valley geometry only; the face midpoint is a disclosed convention and cannot establish a subcell thalweg |
| `hydrology.seeded_face_path_arclength_error.v1` | provisional R1a gate | absolute difference between selected-face polyline length and analytic reference length, divided by analytic length | ratio; one seeded path | tests geometry encoding without cell-centre graph length; endpoint/portal discretization remains explicit |
| `hydrology.seeded_face_path_backtracking_km.v1` | provisional R1a gate | sum of positive upstream-coordinate increments along a nominally downstream selected-face polyline | km; one seeded path | detects loops/detours hidden by cross-track error; depends on the registered analytic direction |
| `hydrology.local_receiver_margin.v1` | provisional R1a diagnostic | `(best-second)/max(|best|, epsilon)` reported separately for physical grade and MFD face fraction at every visited cell | ratio; path steps | numerical dominance, not physical confidence; grade and fraction margins may not be merged or replaced by cumulative path cost |
